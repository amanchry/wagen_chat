...
def wagen_report(self, area, start, stop, precip, et, wri_data,current_user):
    print('startYear:')
    print(start)
    print('endYear:')
    print(stop)
    ...
    GISDBASE = settings.GRASS_DB 
    LOCATION_NAME = settings.GRASS_LOCATION    
    MAPSET = f"job_{jobid}"
#     MAPSET = settings.GRASS_MAPSET
...
#     with Session(gisdb=GISDBASE, location=LOCATION_NAME, mapset="PERMANENT"):
#         new_mapset = os.path.join(GISDBASE, LOCATION_NAME, MAPSET)
#         os.makedirs(new_mapset)
#         # Copy the default WIND file from PERMANENT
#         shutil.copy(
#             os.path.join(GISDBASE, LOCATION_NAME, "PERMANENT", "WIND"),
#             os.path.join(new_mapset, "WIND")
#         )
#         print(f"Mapset '{MAPSET}' created and initialized.")


#     user = Session()


#     user.open(gisdb=GISDBASE, location=LOCATION_NAME,
#                    mapset=MAPSET)
...
#     v.in_ogr(input="PG:dbname={db} host={add} port={po} user={us} password={pwd}".format(db=settings.DATABASES['default']['NAME'], add=settings.DATABASES['default']['HOST'], po=settings.DATABASES['default']['PORT'],  us=settings.DATABASES['default']['USER'], pwd=settings.DATABASES['default']['PASSWORD']), output=vectname, where="id={}".format(area))
    ...
    dem=raster2numpy("dem_studyarea", mapset=MAPSET)
    ...
    LC=raster2numpy("LC_studyarea", mapset=MAPSET)
    ...
    #print(etmaps)
    ETa=raster2numpy('ETa_mean', mapset=MAPSET)
    ...
    #print(etmaps)
    ETr=raster2numpy('ETr_mean', mapset=MAPSET)
...
#     #print(etmaps)
#     ndvimean=raster2numpy('ndvi_mean', mapset=MAPSET)
...
#     plt.savefig(ndvimapplt, bbox_inches='tight',pad_inches = 0, dpi=100)
    ...
    ETano=raster2numpy('etanomap', mapset=MAPSET)
    ...
    #print(etmaps)
    PCP=raster2numpy("pcp_mean", mapset=MAPSET)
    ...
    pminuset=raster2numpy("pminuset", mapset=MAPSET)
    ...
    ax.annotate('N', xy=(x, y), xytext=(x, y-arrow_length),
            ...
            ha='center', va='center', fontsize=20, xycoords=ax.transAxes)
    #ax.legend(bbox_to_anchor=(0.17,0.2))
    plt.colorbar(shrink=0.50, label='P- ETa [mm/year]')
    plt.xlabel('Longitude ($^{\circ}$ East)', fontsize=12)  # add axes label
    plt.ylabel('Latitude ($^{\circ}$ North)', fontsize=12)
    ...
    dmp=raster2numpy("dmp_mean", mapset=MAPSET)
    ...
    ETb=raster2numpy('ETb_mean', mapset=MAPSET)
    ETb = np.ma.masked_where(ETb == -2147483648, ETb)
    ETg=raster2numpy('ETg_mean', mapset=MAPSET)
    ...
    if et == 'wapor2' or et == 'wapor3':
            ...
            Ea=raster2numpy('E_mean_crop', mapset=MAPSET)
            Ea = np.ma.masked_where(Ea == -2147483648, Ea)
            Ta=raster2numpy('T_mean_crop', mapset=MAPSET)
    ...
    WPdmp=raster2numpy('WPdmp', mapset=MAPSET)
    ...
    rwd=raster2numpy('rwd', mapset=MAPSET)
    ...
    bws=raster2numpy('bws', mapset=MAPSET)
    ...
    gws=raster2numpy('gws', mapset=MAPSET)
    ...
    ews=raster2numpy('ews', mapset=MAPSET)
...
#     nta = list(lc_ta_stats.keys())
    ...
    ax.invert_yaxis()
    ax.set_title('Annual ETa/P per Landcover')
...
#     print(persiann)
    ...
    df_comparison = pd.DataFrame({'Year': years, 'chirps': chirps, 'gpm': gpm, 'gsmap': gsmap, 'era5': era5, 'ensemble': ensemble}, index=years)
    ...
    fig, ax = plt.subplots()
    df_comparison.plot.bar(y = ['chirps', 'gpm', 'gsmap', 'era5', 'ensemble'], rot = 40, ax = ax, color=['dodgerblue', 'dimgrey', 'mediumorchid', 'teal', 'navy', 'azure'])
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    #ax.invert_yaxis()
...
#     plt.fill_between(x, df_cc['prssp585_un'], df_cc['prssp585_ov'], color='b', alpha=.1)
    ...
    if wri_data=='true':
        from .wri_data_report import wri_data_report
        wri_data_report(
               mapset=MAPSET,
               shapefile=out2,
               vectname=vectname,
               spatial_extent=spatial_extent,
                output_folder=os.path.join(newdir, "wri_data")
               )
...
#     windsrc = os.path.join(mapset_path, 'WIND')
